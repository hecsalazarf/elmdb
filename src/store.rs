...
use lmdb::{
    Cursor, Database, Environment, Error, Iter, Result, RoCursor, RwTransaction, Transaction,
    WriteFlags,
};
...
impl<D> Store<D> {
    ...
    where
        ...
        T: Transaction,
    {
        txn.get_opt(self.database, &key.as_ref())
            .map(|opt| opt.is_some())
    }
        ...
        T: Transaction,
    {
        txn.open_ro_cursor(self.database)
            .map(|cursor| StoreIter::start(cursor))
    }
    ...
    {
        txn.open_ro_cursor(self.database)
            .map(|cursor| StoreIter::from(cursor, key))
    }
    ...
    {
        txn.open_ro_cursor(self.database)
            .map(|cursor| StoreIter::range(cursor, range))
    }
    ...
    {
        txn.open_ro_cursor(self.database)
            .map(|cursor| StoreIter::end_backwards(cursor))
    }
